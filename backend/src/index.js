...
function assertProductionSecrets() {
	...
	// Пары из env.example лежали в открытом репозитории и годятся любому, кто
	// его видел. Но это слабый секрет, а не обход авторизации: пара обязана
	// совпадать с адресом плейсмента Bitrix, и отказ стартовать сделал бы
	// систему неразворачиваемой без правки настроек в Bitrix. Поэтому громко
	// предупреждаем, а не падаем — критичные дыры закрыты независимо от неё.
	const leaked = String(process.env.PUBLIC_TOKEN_PAIRS)
		.split(',').map((s) => s.trim()).filter((s) => LEAKED_TOKEN_PAIRS.has(s));
	if (leaked.length) {
		console.warn(
			`ВНИМАНИЕ: пары виджета ${leaked.join(', ')} опубликованы в репозитории и известны всем, ` +
			'кто его видел. Заведите новые в PUBLIC_TOKEN_PAIRS и в адресе плейсмента Bitrix.'
		);
	}
